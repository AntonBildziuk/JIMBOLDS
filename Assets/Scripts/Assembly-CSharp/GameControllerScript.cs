...
{
    private bool IsExits0;
    private bool IsExits1;
    private bool IsExits2;
    private bool IsExits3;
    private bool IsExits4;
    public float ItemSlotPosition;
    public Color NotAsRed;
	...
	private void Update()
	{
        ...
        {
            ray0 = Camera.main.ScreenPointToRay(new Vector3((float)(Screen.width / 2), (float)(Screen.height / 2), 0f));

            if (Physics.Raycast(ray0, out raycastHit0))
            {
                if (raycastHit0.transform.name == "Pickup_EnergyFlavoredZestyBar" & Vector3.Distance(this.playerTransform.position, raycastHit0.transform.position) < 10f)
                {
                    Destroy(raycastHit0.transform.gameObject);
                    this.CollectItem(1);
                }
                else if (raycastHit0.transform.name == "Pickup_YellowDoorLock" & Vector3.Distance(this.playerTransform.position, raycastHit0.transform.position) < 10f)
                {
                    Destroy(raycastHit0.transform.gameObject);
                    this.CollectItem(2);
                }
                else if (raycastHit0.transform.name == "Pickup_Key" & Vector3.Distance(this.playerTransform.position, raycastHit0.transform.position) < 10f)
                {
                    Destroy(raycastHit0.transform.gameObject);
                    this.CollectItem(3);
                }
                else if (raycastHit0.transform.name == "Pickup_BSODA" & Vector3.Distance(this.playerTransform.position, raycastHit0.transform.position) < 10f)
                {
                    Destroy(raycastHit0.transform.gameObject);
                    this.CollectItem(4);
                }
                else if (raycastHit0.transform.name == "Pickup_Quarter" & Vector3.Distance(this.playerTransform.position, raycastHit0.transform.position) < 10f)
                {
                    Destroy(raycastHit0.transform.gameObject);
                    this.CollectItem(5);
                }
                else if (raycastHit0.transform.name == "Pickup_Tape" & Vector3.Distance(this.playerTransform.position, raycastHit0.transform.position) < 10f)
                {
                    Destroy(raycastHit0.transform.gameObject);
                    this.CollectItem(6);
                }
                else if (raycastHit0.transform.name == "Pickup_AlarmClock" & Vector3.Distance(this.playerTransform.position, raycastHit0.transform.position) < 10f)
                {
                    Destroy(raycastHit0.transform.gameObject);
                    this.CollectItem(7);
                }
                else if (raycastHit0.transform.name == "Pickup_WD-3D" & Vector3.Distance(this.playerTransform.position, raycastHit0.transform.position) < 10f)
                {
                    Destroy(raycastHit0.transform.gameObject);
                    this.CollectItem(8);
                }
                else if (raycastHit0.transform.name == "Pickup_SafetyScissors" & Vector3.Distance(this.playerTransform.position, raycastHit0.transform.position) < 10f)
                {
                    Destroy(raycastHit0.transform.gameObject);
                    this.CollectItem(9);
                }
                else if (raycastHit0.transform.name == "Pickup_BigBoots" & Vector3.Distance(this.playerTransform.position, raycastHit0.transform.position) < 10f)
                {
                    Destroy(raycastHit0.transform.gameObject);
                    this.CollectItem(10);
                }
                else if (raycastHit0.transform.name == "Pickup_GameBoy" & Vector3.Distance(this.playerTransform.position, raycastHit0.transform.position) < 10f)
                {
                    Destroy(raycastHit0.transform.gameObject);
                    this.CollectItem(11);
                }
    ...
    MathGame = GameObject.FindGameObjectWithTag("MathGame");
        /*                 int RadNum;
                RadNum = UnityEngine.Random.Range(0, 2);
                this.audioDevice.clip = this.aud_MachineRev[RadNum]; */
        if (IsExits0)
        {
            if(MathGame == null)
            {
        ...
        if(IsExits1)
        {
            ...
            {
                this.audioDevice.clip = this.aud_MachineRev[0];
                this.audioDevice.Play();
                IsExits2 = true;
                IsExits1 = false;
            }
        ...
        if (IsExits2)
        {
            if (!audioDevice.isPlaying)
            {
                this.audioDevice.clip = this.aud_MachineRev[1];
        ...
        if(IsExits4)
        {
            ...
            {
                int RadNum;
                RadNum = UnityEngine.Random.Range(0, 3);
                this.audioDevice.clip = this.aud_MachineRev[RadNum];
                this.audioDevice.Play();
            }
        }
		...
		if (this.player.gameOver)
		{
            ...
            if(!hasPLayed)
            {
                GameOverSound.PlayOneShot(this.aud_buzz);
                hasPLayed = true;
            }
	...
	private void UpdateNotebookCount()
	{
		if (this.mode == "story")
		{
			this.notebookCount.text = this.notebooks.ToString() + "/7 Magical Scrolls";
		}
		else
		{
			this.notebookCount.text = this.notebooks.ToString() + " Magical Scrolls";
		}
	...
	private void IncreaseItemSelection()
	{
		...
		this.itemSelect.anchoredPosition = new Vector3(ItemSlotPosition * itemSelected + -123.1f, -12.6f, 0f); //Moves the item selector background(the red rectangle)
		this.UpdateItemName();
	}
	...
	public void CollectItem(int item_ID)
	{
		...
		{
            GameObject ItemDroppped = Instantiate(ItemPickUpPrefabs[item[itemSelected] - 1], raycastHit0.transform.position, raycastHit0.transform.rotation); // creates a prefab of the pickup in the inventory and puts it into the same position as the picked up item position
            ItemDroppped.name = ItemDroppped.name.Replace("(Clone)", "").Trim();
	...
	public void ExitReached()
	{
		...
		{
            RenderSettings.ambientLight = NotAsRed; //Make everything ourple and start player the weird sound
