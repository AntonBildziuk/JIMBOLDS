...
public class PlayerScript : MonoBehaviour
{
	...
	private void OnTriggerEnter(Collider other)
	{
		...
		{
            if(this.gc.item[0] != 11 & this.gc.item[1] != 11 & this.gc.item[2] != 11)
            {
                this.gameOver = true;
            ...
            {
                GameObject baldiGameBoy = Instantiate(baldiEat, baldi.gameObject.transform.position, baldi.gameObject.transform.rotation);
                baldi.gameObject.SetActive(false);
                int num = Mathf.RoundToInt(UnityEngine.Random.Range(0f, 2f)); //Get a random item slot
                while (this.gc.item[num] != 11) //If the selected slot is not the Game Boy
                {
